...
(define (part-1 input)
  (for/fold ([total-joltage 0]) ([bank (in-list input)])
    (match-define (list bank-init ... bank-last) (map char->number (string->list bank)))
    (define-values (digit-1 digit-2)
      (for/fold ([digit-1 -1]
                 [digit-2 -1])
                ([digit-current (in-list bank-init)])
        (cond
          [(< digit-1 digit-current) (values digit-current -1)]
  ...
  (for/fold ([total-joltage 0]) ([bank (in-list input)])
    (define-values (bank-joltage _)
      (for/fold ([bank-joltage 0]
        ...
        (define-values (batteries-pool batteries-rest)
          (split-at bank (- (length bank) batteries-to-turn-on)))
...
(module+ test
  ...
  (check part-2
         ...
         `(3121910778619
           ,(string-join '("987654321111111" "811111111111119" "234234234234278" "818181911112111")
                         "\n"))))
